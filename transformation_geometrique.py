# Importation des modules nécessaires pour le calcul des transformations
...
#l'axe est 'x', la réflexion se produit par rapport à l'axe vertical (mirroir horizontal),
#changeant la coordonnée y du point.
    if axe == 'x':
        y = -y
#Si l'axe est 'y', la réflexion se produit par rapport à l'axe horizontal (mirroir vertical),
# changeant la coordonnée x.
    else:
        x = -x
...
faire pivoter.
    ...
    centre (tuple) : Un tuple représentant les coordonnées du centre de
...
#m=tan(angle) est la tangente de l'angle d'inclinaison, et cette valeur
